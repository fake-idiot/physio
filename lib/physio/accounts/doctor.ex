defmodule Physio.Accounts.Doctor do
  use Ecto.Schema
  ...
  def doctor_update_changeset(doctor, attrs, opts \\ []) do
    doctor
    |> cast(attrs, [:email, :password])
    |> cast_assoc(:doctor_profile, with: &DoctorProfile.changeset/2, required: true)
  ...
  """
  def valid_password?(%Physio.Accounts.Doctor{hashed_password: hashed_password}, password)
